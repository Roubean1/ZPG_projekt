...
void Application::key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS) {
        if (app->getIsDragging()) {
            ...
            std::cout << "Moving cancelled" << std::endl;
        }
        else if (app->splineDrawingMode) {
            app->splineDrawingMode = false;
            std::cout << "Drawing mode cancelled" << std::endl;
        }
        else {
    ...
    if (key == GLFW_KEY_Z && action == GLFW_PRESS) {
        ...
        app->toggleSplineDrawingMode();
    }
    else if (key == GLFW_KEY_N && action == GLFW_PRESS) {
        app->startSplineMovement();
    }
    else if (key == GLFW_KEY_C && action == GLFW_PRESS) {
        app->clearSplinePoints();
    }
}
...
void Application::createScenes() {
    ...
    scenes.push_back(std::make_shared<RaceTrackScene>(camera));
...
void Application::toggleSplineDrawingMode() {
    splineDrawingMode = !splineDrawingMode;
    if (splineDrawingMode) {
        std::cout << "Drawing mode activated" << std::endl;
        std::cout << "B exit, N start movement, C clear points." << std::endl;
    }
    else {
        std::cout << "Spline drawing deactivated" << std::endl;
    }
}

void Application::addSplineControlPoint(const glm::vec3& worldPos) {
    if (!splineDrawingMode) return;

    splineControlPoints.push_back(worldPos);
    std::cout << "Added  " << splineControlPoints.size() << " at: " << worldPos.x << ", " << worldPos.y << ", " << worldPos.z << std::endl;

    if (splineControlPoints.size() >= 4) {
        bezierSplineMovement = std::make_shared<BezierSplineMovement>(splineControlPoints, 0.5f, true);
        std::cout << "Spline movement " << splineControlPoints.size()<< " points." << std::endl;
    }
}

void Application::startSplineMovement() {
    if (splineControlPoints.size() < 4) {
        std::cout << "4 control points!" << std::endl;
        return;
    }

    auto phongShader = ShaderManager::getShader("phong");
    auto rotatingMaterial = std::make_shared<Material>(
    ...
    auto rotatingSplineMovement = std::make_shared<BezierSplineMovement>(
        splineControlPoints, 0.5f, true, true);

    auto rotatingObject = std::make_shared<DrawableObject>(
        sphere, sizeof(sphere) / (6 * sizeof(float)), 6,
        phongShader, rotatingMaterial
    );

    rotatingObject->getTransformations()->addTransform(rotatingSplineMovement);
    rotatingObject->getTransformations()->addTransform(std::make_shared<Scale>(glm::vec3(0.3f)));

    auto staticMaterial = std::make_shared<Material>(
    ...
    auto staticSplineMovement = std::make_shared<BezierSplineMovement>(
        splineControlPoints, 0.5f, true, false);

    auto staticObject = std::make_shared<DrawableObject>(
        sphere, sizeof(sphere) / (6 * sizeof(float)), 6,
        phongShader, staticMaterial
    );

    staticObject->getTransformations()->addTransform(staticSplineMovement);
    staticObject->getTransformations()->addTransform(std::make_shared<Scale>(glm::vec3(0.2f)));

    auto currentScene = getCurrentScene();

    rotatingObject->setId(currentScene->getNextObjectId());
    currentScene->addObject(rotatingObject);

    staticObject->setId(currentScene->getNextObjectId());
    currentScene->addObject(staticObject);

    std::cout << "Spline move!" << std::endl;
}

void Application::clearSplinePoints() {
    splineControlPoints.clear();
    bezierSplineMovement.reset();
    std::cout << "Spline control points cleared." << std::endl;
}
